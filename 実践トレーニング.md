...
1. https://git-scm.com/download/win からインストーラをダウンロードして実行
...
Claude Code には **2 つのインストール方法**がある。どちらでも同じ機能が使えるので、環境に合った方を選ぶ。

#### 方法 A：VS Code 拡張機能からインストール（推奨）
...
3. インストール完了後、VS Code のターミナルを **開き直して** 以下のコマンドで確認する
...
**手順 3：VS Code 拡張機能のインストール（方法 B でも必要）**
...
> **方法 B のメリット**: ターミナルで `claude` コマンドを直接実行できるため、拡張機能のパネルが重いと感じる場合はターミナルから起動する方が快適に動作する。VS Code のターミナルで `claude` と入力すれば、拡張機能を経由せずに Claude Code を利用できる。
