...
4. 左側の言語一覧から **「日本語 (ja)」** の `Install` ボタンをクリック
5. 右下に出る `Restart` をクリック → VS Code が日本語表示で再起動される

> 日本語化されない場合は、もう一度 `Ctrl+Shift+P` →「`Configure Display Language`」→「`日本語 (ja)`」を選び直してから VS Code を再起動してください。

### 1-3. ターミナル（PowerShell）から `code` コマンドを使えるようにする

VS Code は PowerShell から `code <ファイル名>` で開けるようにしておくと、Claude Code が作った `.md` ファイルをすぐにプレビューできて便利です。

Windows の VS Code インストーラはデフォルトで **`PATH` への追加** が有効になっているため、通常はインストール直後から `code` コマンドが使えます。

1. **PowerShell を一度閉じてから開き直す**（インストール前に開いていた PowerShell では PATH が反映されません）
2. 以下を実行して動作確認
...
> **`code` is not recognized` と出る場合**: VS Code インストーラで「`PATH` への追加」のチェックが外れていた可能性があります。VS Code を起動 → `Ctrl+Shift+P` → `Shell Command: Install 'code' command in PATH` を実行してから、PowerShell を開き直してください。

> **動作確認**: 研修用フォルダがあれば、PowerShell から以下で開けます。
>
> ```powershell
> code .                       # 今いるフォルダ全体を VS Code で開く
> code memo.md                 # ファイル単体を開く
> ```
...
### 1-7. Claude Code 本体のインストール（PowerShell ネイティブインストーラ）
...
### 1-8. VS Code 拡張機能のインストール（IDE 連携用）
...
> 拡張機能は手順 1-7 でインストールした `claude.exe` 本体と連携して動作します。本体が入っていない状態で拡張機能だけ入れても動きません。
...
> テキストに簡単な記号を付けて見出しや箇条書きを表現する軽量な文書形式（拡張子 `.md`）。本勉強会では、Claude Code に作らせるドキュメント（議事録・競合調査・ニュースレター・提案書の下書きなど）はすべて Markdown で扱う。
>
> **なぜ Markdown を使うのか**
...
PDF や Word ファイルを直接編集するのではなく、**markdown ファイルを編集してから変換する**のが効率的なワークフロー（理由は 2-1 の「なぜ Markdown を使うのか」を参照）。
...
> **Markdown のプレビュー方法（VS Code）**: VS Code は標準で Markdown プレビュー機能を持っているため、追加のインストールは不要です。
>
> - `.md` ファイルをエディタで開いた状態で `Ctrl+Shift+V` … 別タブにプレビューを開く
> - `.md` ファイルをエディタで開いた状態で `Ctrl+K` → `V` … 右側に **エディタとプレビューを並べて表示**（修正と確認を同じ画面でできるのでおすすめ）
> - PowerShell から `code 競合比較表.md` で直接ファイルを開ける（手順 1-3 で `code` コマンドを有効化済みの場合）
>
> Claude Code に修正を依頼するたびに、左側のテキストと右側のプレビューが自動更新されるため、編集と確認を同じ画面で完結できます。
...
本編 1-7 では PowerShell からのネイティブインストールを案内したが、Anthropic は他にもいくつかの公式インストール方法を用意している。社内の方針や既存の環境に合わせて選んでほしい。
...
> `The token '&&' is not a valid statement separator` というエラーが出たら、PowerShell を開いてしまっている。CMD を開き直すか、本編 1-7 の PowerShell 用コマンドを使う。
...
> ⚠️ 管理者権限の昇格 PowerShell での実行は **避ける**。権限エラーが出たら、ユーザー権限のままで `npm config set prefix` を変更するか、本編 1-7 のネイティブインストーラに切り替えるほうが安全。
...
A-1〜A-3 のいずれでインストールしても、その上に VS Code 拡張機能を入れる手順は本編 1-8 と同じ。本体（CLI）と拡張機能はセットで使うイメージ。
