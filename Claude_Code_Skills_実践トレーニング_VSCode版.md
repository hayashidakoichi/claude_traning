...
Claude Code をターミナルで使う際、以下の2つのキー操作を覚えておくと安全に操作できます。

| キー操作 | 用途 | 説明 |
|---|---|---|
| **Esc（エスケープキー）** | Claude の応答を途中で止める | Claude が回答や処理を実行している最中に押すと、その動作を中断できます。意図しない方向に進み始めたときに使います。 |
| **Ctrl + C（Mac は Command + C ではなく Control + C）** | 入力のキャンセル / Claude Code の終了 | 入力途中の文字をクリアしたいときに1回押します。何も入力していない状態で2回続けて押すと、Claude Code 自体を終了できます。 |

> ⚠️ **注意**: ターミナルでは Mac でも **Command + C ではなく Control + C** を使います。Command + C は通常のコピー操作なので、間違えないようにしましょう（ターミナル上での文字コピーは選択するだけで自動コピーされる、または右クリックメニューから行います）。
...
表の中に出てくる `~/.claude/` や `<project>/.claude/` がどこを指すのか、Windows / Mac 別の実体パスを押さえておくと、スキルを探したり手動コピーするときに迷いません。
...
**エクスプローラー / Finder で開く方法**

- **Mac（Finder）**：`Cmd + Shift + G` を押して `~/.claude` と入力して Enter
  - `.` で始まるフォルダは通常非表示です。Finder 上で見えないときは `Cmd + Shift + .`（ピリオド）で隠しファイルの表示をトグルできます
- **Windows（エクスプローラー）**：アドレスバーに `%USERPROFILE%\.claude` と入力して Enter
  - もしくは「表示」→「隠しファイル」にチェックを入れてから `C:\Users\<ユーザー名>` を開く

**ターミナル / PowerShell で開く方法**

- **Mac（ターミナル）**：`open ~/.claude`
- **Windows（PowerShell）**：`explorer $env:USERPROFILE\.claude`
- **Windows（Git Bash）**：`explorer "$(cygpath -w ~/.claude)"`

### プロジェクト領域（`<project>/.claude/`）

今開いているプロジェクトフォルダの直下に作られる `.claude/` です。チームで Git 共有するスキルはここに置きます。

- **Mac / Windows 共通**：Claude Code を起動したプロジェクトフォルダの直下
  - 例：`/Users/khayashida/development/business_plannings/claude_traning/.claude/`
  - 例：`C:\Users\taro\projects\sample-app\.claude\`
- VS Code で開いている場合は、左のエクスプローラーで「隠しファイルを表示」設定にしておくと `.claude/` がツリーに出てきます

### よくあるハマりどころ

- **`.`（ドット）で始まるフォルダは隠しフォルダ扱い** → 設定を変えないと Finder / エクスプローラーに出てきません
- **ユーザー領域とプロジェクト領域を混同しがち** → 「自分だけ全プロジェクトで使いたい」なら `~/.claude/`、「このプロジェクトのチーム全員で使いたい」ならプロジェクト直下の `.claude/`
- **Windows の `~` は PowerShell だと効かないことがある** → `$env:USERPROFILE` を使うのが確実
