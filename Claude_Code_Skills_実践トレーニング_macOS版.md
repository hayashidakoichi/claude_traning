...
# Claude Code Skills 実践トレーニング（macOS + ターミナル版）
...
1. **ターミナルアプリを起動する**
   - Finder で「アプリケーション → ユーティリティ → ターミナル.app」をダブルクリック
   - もしくは Spotlight（`Cmd + Space`）で「ターミナル」と入力して Enter
   - Dock に登録しておくと毎回探さずに済むので便利
...
> `mkdir -p` は「無ければフォルダを作る」コマンド。`cd` は「そのフォルダに移動する」コマンド。
> 場所はどこでも構いませんが、迷ったら Desktop（デスクトップ）配下が分かりやすいです。
...
→ `/Users/<ユーザー名>/Desktop/claude-training` が表示されればOK。

---

## ターミナルで claude コマンドを実行する

研修用フォルダに `cd` で移動した状態のまま、以下を実行します。

```ターミナル
claude
```

初回は認証画面（ブラウザ）が開くので、画面の指示に従ってログインしてください。

> ⚠️ ターミナルの **作業ディレクトリ（今いる場所）** が、Claude Code の作業対象になります。`pwd` で位置を確認してから `claude` を起動するクセを付けると、意図しないフォルダで作業してしまう事故を防げます。

---

## ターミナルの使い方（覚えておくと便利なキー操作）

Claude Code をターミナルで使う際、以下のキー操作を覚えておくと安全に操作できます。

| キー操作 | 用途 | 説明 |
|---|---|---|
| **Esc（エスケープキー）** | Claude の応答を途中で止める | Claude が回答や処理を実行している最中に押すと、その動作を中断できます。意図しない方向に進み始めたときに使います。 |
| **Control + C** | 入力のキャンセル / Claude Code の終了 | 入力途中の文字をクリアしたいときに1回押します。何も入力していない状態で2回続けて押すと、Claude Code 自体を終了できます。 |
| **Cmd + K** | 画面のクリア（ターミナル.app） | 画面が散らかってきたときにすっきりさせるためのターミナル側の機能。Claude Code の会話履歴は消えません。 |
| **Cmd + T** | 新しいタブを開く（ターミナル.app） | Claude Code を動かしたまま、別タブで `git` などのコマンドを叩きたいときに使います。 |

> ⚠️ **注意**: macOS のターミナルでは、Claude Code への割り込みは **Command + C ではなく Control + C** です。Command + C は OS 標準のコピー操作（選択範囲をクリップボードにコピー）なので、間違えると Claude が止まらず焦ります。
>
> 逆に「ターミナル上に表示されている文字をコピーしたい」ときは、対象を **マウスで選択 → Command + C**、または右クリックメニューから「コピー」を選びます。役割を切り分けて覚えましょう。
...
## 【参考】　`.claude` フォルダはどこにある？

表の中に出てくる `~/.claude/` や `<project>/.claude/` がどこを指すのか、macOS 上での実体パスを押さえておくと、スキルを探したり手動コピーするときに迷いません。

### ユーザー領域（`~/.claude/`）

全プロジェクト共通の個人設定・スキル・プラグインキャッシュが入っている場所です。

- **実体パス**：`/Users/<ユーザー名>/.claude/`
- `~`（チルダ）は「自分のホームディレクトリ」を指すショートカット。ターミナルで `echo ~` を打つと実際のパスを確認できます。

**Finder で開く方法**

1. Finder を開く
2. `Cmd + Shift + G` を押して「フォルダへ移動」を表示
3. `~/.claude` と入力して Enter

> `.` で始まるフォルダは通常非表示です。Finder 上で `.claude/` の中身が見えないときは、`Cmd + Shift + .`（ピリオド）で隠しファイルの表示をトグルできます。
...
> `open <パス>` は macOS 専用の便利コマンド。ファイル / フォルダを Finder や関連付けされたアプリで開きます。

### プロジェクト領域（`<project>/.claude/`）

今開いているプロジェクトフォルダの直下に作られる `.claude/` です。チームで Git 共有するスキルはここに置きます。

- Claude Code を起動したプロジェクトフォルダの直下
- 例：`/Users/khayashida/development/business_plannings/claude_traning/.claude/`
...
# プロジェクトフォルダに移動した状態で
cd ~/Desktop/claude-training
open .claude        # Finder で開く
ls -la .claude      # ターミナルで一覧表示
```

### よくあるハマりどころ

- **`.`（ドット）で始まるフォルダは隠しフォルダ扱い** → 設定を変えないと Finder には出てきません（`Cmd + Shift + .` で表示切り替え）
- **ユーザー領域とプロジェクト領域を混同しがち** → 「自分だけ全プロジェクトで使いたい」なら `~/.claude/`、「このプロジェクトのチーム全員で使いたい」ならプロジェクト直下の `.claude/`
- **`open` が「ファイルを開く」なのか「Finder で開く」なのか迷う** → 引数がフォルダなら Finder で、ファイルなら関連付けされたアプリで開きます。フォルダを Finder で確実に開きたいときは `open .` で「今いるフォルダ」を開けます
...
```Claude Code プロンプト
先ほどpluginでインストールしたinternal-commsをプロジェクトフォルダの.claude/skills配下にコピーして
```
...
うまく行かない場合の対処法は大きく2つあります。

1. **スラッシュコマンドで強制的に発火させる**
2. **CLAUDE.md に発火条件を書いておく**

---

## 対処1: スラッシュコマンドで強制発火

Claude の自動判断に任せず、**こちらから直接スキルを指定して起動**します。
...
- キーワードマッチに頼らないので **確実**
- その場限りの起動なので、毎回自分で打つ必要あり
- 「今このタスクで絶対に使ってほしい」ときに便利

---

## 対処2: CLAUDE.md に発火条件を書く

CLAUDE.md に **「どういうときにどのスキルを使うか」** を明記しておくと、Claude はそれを毎回のコンテキストとして読み込むため、自動発火しやすくなります。

```markdown
## skills

- ニュースレターを作る時には skills の "internal-comms" を使って下さい。
- 議事録を整形する時には skills の "meeting-notes" を使って下さい。
```

- **一度書けば、以降の会話すべてに効く**
- チームでリポジトリを共有していれば、メンバー全員に同じルールが適用される
- スキルの `description` を直さなくても運用で補える

→ **その場しのぎならスラッシュコマンド、恒久対応なら CLAUDE.md** と覚えておきましょう。
...
## まず大原則：何でも CLAUDE.md に詰め込まない

個別の比較に入る前に、もっと大きな視点を1つ。

Claude Code には、AI の動きを指示する方法が複数あります（CLAUDE.md・スキル・サブエージェント・ルール・フック等）。便利だからといって **何でも CLAUDE.md に書くと、ファイルが肥大化** します。

CLAUDE.md は **起動時に毎回・全員・全タスクで読み込まれる** ため、肥大化すると——

- トークンを無駄づかいする（読まなくていい指示まで毎回読む）
- **本当に守ってほしい指示への遵守度が薄まる**（指示が多すぎて埋もれる）

→ 公式の結論は「**1 か所に集めず、その指示の“使う範囲”と“使う頻度”に合った置き場所に分散させる**」。CLAUDE.md は **200 行以内** に保ち、オーナーを決め、コードと同じようにレビューするのが推奨です。

出典: *Steering Claude Code: skills, hooks, rules, subagents, and more* — Anthropic 公式ブログ
...
1. **いつ読まれるか** … 起動時 / 必要時 / 呼んだ時
2. **コンパクション（文脈の自動圧縮）で残るか**
3. **コンテキスト（トークン）コスト** … 高 / 中 / 低

| 仕組み | いつ読まれる | コスト | 何に使う |
|---|---|---|---|
| **CLAUDE.md** | 起動時（サブ階層は必要時） | 高 | ビルドコマンド、構成、規約、チームの決まり |
| **ルール (Rules)** | 起動時／パス限定時 | 中 | 「API は必ず入力検証」等の個別制約。対象パスを絞れる |
| **スキル (Skills)** | 名前と説明は起動時、本体は呼んだ時 | 低 | デプロイ・リリース等の **手順書（プレイブック）** |
| **サブエージェント** | 名前と説明は起動時、本体は呼んだ時 | ほぼ 0 | 検索・ログ解析など **隔離した別作業**。結果だけ返る |
| **フック (Hooks)** | 編集・コマンド実行などの節目 | 低 | 編集後に自動でリンター、危険コマンドの **機械的ブロック** |
| **出力スタイル** | 起動時、システムプロンプトに注入 | 高 | 役割そのものを変える大改造のみ（普段は使わない） |
| **システムプロンプト追記** | 起動時、CLI フラグで | 中 | 特定の規約・出力形式・ドメイン知識 |

> **受講者向けメモ：** まず使うのは上 3 つ（CLAUDE.md / スキル / サブエージェント）。**ルール・フック・出力スタイルは上級者向け** で、いまは「存在を知っておく」だけで十分です。

出典: *Steering Claude Code* — Anthropic 公式ブログ

---

## 誤用 → 正しい置き場所

「CLAUDE.md に書いたのにうまくいかない」典型例と、その正解です。

| やりたいこと | ありがちな間違い | 正解 |
|---|---|---|
| 「X したら必ず Y せよ」 | CLAUDE.md に書く（お願いベース） | **フック**（機械的に毎回実行） |
| 「絶対にこれをするな」 | CLAUDE.md に書く | **フックで拒否**（実行を機械的に止める） |
| 30 行のデプロイ手順 | CLAUDE.md に常駐させる | **スキル**（呼んだ時だけ読み込む） |
| 特定フォルダだけの制約 | 範囲指定なしのルール | **パス限定ルール**（対象パスを絞る） |
| 個人の好み | プロジェクトの CLAUDE.md に書く | **個人設定**（`~/.claude/`）へ |

→ ポイントは「**“毎回必ず”や“絶対禁止”は、お願い（プロンプト）ではなく機械的な強制（フック）で担保する**」こと。お願いベースの指示は、AI が必ず守るとは限りません。

出典: *Steering Claude Code* — Anthropic 公式ブログ

---

## 【ハンズオン】自分の設定を記事の設計で点検する

学んだ設計指針に、**自分の今の設定が合っているか** を Claude Code 自身に点検してもらいましょう。下のプロンプトをそのまま貼り付けて実行します。

```text
このページを読んで、その設計指針に照らして私の Claude Code 設定を点検してください。
https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more

点検対象:
- ~/.claude/CLAUDE.md（グローバル設定）
- このプロジェクトの ./CLAUDE.md と ./.claude/ フォルダ（ローカル設定）

次の観点でチェックしてください:
1. CLAUDE.md が肥大化していないか（200 行が目安。毎回読まれて損していないか）
2. 「毎回必ず」「絶対にするな」をお願いベースで書いていないか（→ フック向き）
3. 長い手順書を CLAUDE.md に常駐させていないか（→ スキル向き）
4. 特定パスだけの制約を範囲指定なしで書いていないか（→ パス限定ルール向き）
5. 個人の好みがプロジェクトの CLAUDE.md に混ざっていないか（→ 個人設定へ）

各観点を「現状 → 問題点 → どこに移すべきか」の表でまとめ、
最後に具体的な修正案を出してください。
この時点ではまだファイルを変更しないでください。
```

**ポイント：**
- 最後の一文（まだ変更しない）で、**点検だけ** させて結果を自分で確認できます。直すのはそのあと。
- 出てきた指摘は鵜呑みにせず、「これは本当にフックにすべき？」と一度自分で判断しましょう（上の判断表が材料になります）。
- 直す場合は 1 か所ずつ。直したら CLAUDE.md の行数が減っているか確認すると効果が分かります。
...
プロジェクトのルートに置く **「このリポジトリの取扱説明書」** です。Claude Code は起動時にこのファイルを自動で読み込み、以降の会話すべてのコンテキストとして利用します。

書く内容の例:
- このリポジトリが何のためのものか（プロジェクトの性格）
- ディレクトリ構成と、それぞれの役割分担
- よく使うコマンド（ビルド・テスト・リント等）
- コーディング規約・命名ルール・運用ルール
- 受講者／読者の前提知識や用語の使い方

→ **毎回同じ説明をしなくて済む**。新しく入ったメンバーに渡す README の AI 版、というイメージ。

---

## `/init` コマンド

CLAUDE.md をゼロから書くのは面倒なので、Claude Code には **自動生成コマンド `/init`** が用意されています。

```
/init
```

実行すると Claude がリポジトリ全体をスキャンし、

1. README やソースコードを読んでプロジェクトの性格を把握
2. ディレクトリ構成・主要ファイル・使われているコマンドを整理
3. 叩き台となる `CLAUDE.md` をリポジトリ直下に作成

してくれます。**生成後はそのまま使わず、必ず人間が目を通して、プロジェクト固有の事情（暗黙ルール・過去のハマりどころ・用語）を追記するのがコツ**です。

---

## CLAUDE.md の設置場所

CLAUDE.md は複数の階層に置けて、重ねて適用されます:

| 場所 | スコープ | 用途 |
|---|---|---|
| `~/.claude/CLAUDE.md` | 全プロジェクト共通（個人） | 自分の好みの指示・全体ルール |
| `<リポジトリ>/CLAUDE.md` | そのリポジトリ | プロジェクト固有の情報 |
| `<サブディレクトリ>/CLAUDE.md` | そのサブディレクトリ配下 | モノレポで領域ごとに分けたいとき |

→ スコープを分けて書けるので、**「個人の好み」と「プロジェクトのルール」が混ざらない**。
...
## Skillを新規に作成するskill-creatorをインストールする
...
## 【ハンズオン】 Skills の探し方（できるだけ公式から）

スキルは自分で作る前に、**まず公式・準公式のものを探す** のが鉄則です。野良スキルは中身を全部読まないと怖いので、信頼できる入手元を押さえておきましょう。

---

## 探し先1: Anthropic 公式 `example-skills`

Anthropic 自身が配布しているサンプルスキル集。今回のハンズオンで使った `internal-comms` もここに含まれています。

- GitHub: https://github.com/anthropics/skills
- `pptx` / `docx` / `xlsx` / `pdf` / `internal-comms` / `skill-creator` など、業務で即使えるスキルが揃っている
- Claude Code からは **プラグインとしてインストール** するのが一番簡単

```
> /plugin
→ example-skills を選んでインストール
```

---

## 探し先2: Claude Code 公式ドキュメント

公式ドキュメントの Skills ページに、**推奨スキルと使い方のサンプル** が載っています。新しい機能や書き方の正解は、まずここを確認します。
...
## 探し先3: Claude Code Marketplace / Plugin リポジトリ

Claude Code の `/plugin` コマンドから、登録済みの Marketplace を検索できます。

```
> /plugin marketplace
```

- 公式・準公式の Marketplace（anthropics 配下など）を優先
- インストール前に **GitHub リポジトリの運営元・Star 数・更新頻度** を必ず確認
- 第三者の Marketplace を追加する場合は、`.claude-plugin/marketplace.json` の中身まで目を通す

---

## 探し先4: コミュニティの記事・まとめ

新しいスキルは Zenn / Qiita / classmethod / 公式ブログで頻繁に紹介されます。「どんなスキルが流行っているか」のキャッチアップに便利。

- Anthropic 公式ブログ: https://www.anthropic.com/news
- Zenn で「Claude Code Skills」タグを購読
- classmethod DevelopersIO の Claude Code カテゴリ

→ **記事で見つけた → 公式リポジトリの存在を確認 → そこからインストール** の流れが安全。

---

## 【ハンズオン手順】

1. ブラウザで https://github.com/anthropics/skills を開いて、どんなスキルがあるか眺める
2. 気になったスキルの `SKILL.md` を1つ開いて `description` と `examples/` の中身を確認する
3. Claude Code で `/plugin` を実行し、`example-skills` がインストール済みか確認する
4. 未インストールならインストールし、`/` を打って新しく増えたスキルがサジェストされることを確かめる
5. 業務で使えそうなスキルを1つ選び、実際にプロンプトで呼び出してみる

→ **「探す → 中身を読む → 試す」** の3ステップを体験することが目的。いきなり自作しない。
